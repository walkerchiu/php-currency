...
class CurrencyObserver
{
    ...
    public function saving($entity)
    {
        if ($entity->is_base) {
            ...
            config('wk-core.class.currency.currency')::withTrashed()
                ->where('id', '<>', $entity->id)
    ...
    public function deleted($entity)
    {
        ...
        if ( config('wk-currency.onoff.account') && !empty(config('wk-core.class.account.profile')) ) {
            config('wk-core.class.account.profile')::where('currency_id', $entity->currency_id)
                ->update(['currency_id' => null]);
        }
        if ( config('wk-currency.onoff.site') && !empty(config('wk-core.class.site.site')) ) {
            config('wk-core.class.site.site')::where('currency_id', $entity->currency_id)
                ->update(['currency_id' => null]);
        }
        if ( config('wk-currency.onoff.group') && !empty(config('wk-core.class.group.group')) ) {
            config('wk-core.class.group.group')::where('currency_id', $entity->currency_id)
                ->update(['currency_id' => null]);
        }
    }
